...
@Component({
  selector: 'exp-logbook-input-new',
  templateUrl: './logbook-input-new.component.html',
  styleUrls: ['./logbook-input-new.component.scss']
})
export class LogbookInputNewComponent implements OnInit {
  ...
  processForm(): LogbookInputCreation {
    ...
    const logbook: LogbookInputCreation = {
  ...
  submitForm() {
    if (this.form.invalid)
      return;
    const logbook: LogbookInputCreation = this.processForm();
    this.checkPointService.saveInputLogbook(logbook).subscribe(() => {
